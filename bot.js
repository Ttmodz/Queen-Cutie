...
async function Queen-Cutie () {
    ...
    var insult = await axios.get('https://gist.githubusercontent.com/Ttmodz/f16bbd4ceeb4324d4a727b431a4ef1f2/raw')
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(
            chalk.blueBright.italic('💞Connecting to Queen-Cutie Database...')
        );
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [Queen-Cutie] --*' + 
                                    '\n*Queen-Cutie bir hata gerçekleşti!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [Queen-Cutie] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [?Queen Qutie?] --*' + 
                                    '\n*?Queen Qutie? an error has occurred!*'+
                                    '\n*This error log may include your number or the number of an opponent. Please be careful with it!*' +
                                    '\n*സീ�? ഇല്ലാത്ത error ആണ�?.*' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [?Queen Qutie?] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻സീ�? ഇല്ലാത്ത error ആണ്🙇�?*' +
                                        '\n_You can write to our support group for more help /n https://https://wa.me/+94712564306'
                                        , MessageType.text
                                    );
                                }    
                            }                      
