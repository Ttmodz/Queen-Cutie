...
Queen-Cutie - Queen-Cutie
*/

const fs = require("fs");
const path = require("path");
const events = require("./events");
const Queen-Cutie = require('Queen-Cutie-public-2');
const chalk = require('chalk');
const axios = require('axios');
const config = require('./config');
const Heroku = require('heroku-client');
const {WAConnection, MessageOptions, MessageType, Mimetype, Presence} = require('@adiwajshing/baileys');
const {Message, StringSession, Image, Video} = require('./Queen-Cutie');
...
const Queen-CutieDB = config.DATABASE.define('Queen-Cutie', {
    info: {
...
async function Queen-Cutie () {
    await config.DATABASE.sync();
    var StrSes_Db = await Queen-CutieDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
...
const Queen-CutieCon = new WAConnection();
const Session = new StringSession();
Queen-CutieCon.version = [2, 2126, 14]
setInterval(async () => { 
        ...
        await axios.get('https://gist.githubusercontent.com/BlackAmda/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (infoen !== '' && config.LANG == 'EN' || config.LANG == 'ES') {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '[ ```🔔Queen Amdi Announcements🔔``` ]\n\n' + infoen.replace('{user}', Queen-CutieCon.user.name).replace('{wa_version}', Queen-CutieCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', Queen-CutieCon.user.phone.os_version).replace('{device_model}', Queen-CutieCon.user.phone.device_model).replace('{device_brand}', Queen-CutieCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (infosi !== '' && config.LANG == 'SI') {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '[ ```🔔Queen Amdi නිවේදන🔔``` ]\n\n' + infosi.replace('{user}', Queen-CutieCon.user.name).replace('{wa_version}', Queen-CutieCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', Queen-CutieCon.user.phone.os_version).replace('{device_model}', Queen-CutieCon.user.phone.device_model).replace('{device_brand}', Queen-CutieCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
}, 50000);
...
setInterval(async () => { 
    if (config.AUTO_BIO == 'true') {
        var tz_bio = await Queen-Cutie.timezone(Queen-CutieCon.user.jid)
        var date = await Queen-Cutie.datebio(config.LANG)
        const biography = '📅 ' + date + '\n⌚ ' + tz_bio + '    🎖️ ' + config.CAP
        await Queen-CutieCon.setStatus(biography)
    }
}, 7890);
        ...
        await axios.get('https://gist.githubusercontent.com/BlackAmda/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (verify !== '' && config.LANG == 'EN') {
                while (getGMTh == 20 && getGMTm == 55) { 
                    return Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, HANDLER + verify.replace('{user}', Queen-CutieCon.user.name).replace('{wa_version}', Queen-CutieCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', Queen-CutieCon.user.phone.os_version).replace('{device_model}', Queen-CutieCon.user.phone.device_model).replace('{device_brand}', Queen-CutieCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
...
// ======================Queen_Amdi_Logger=======================
Queen-CutieCon.logger.level = config.DEBUG ? 'debug' : 'warn';
var nodb;

    if (StrSes_Db.length < 1) {
        nodb = true;
        Queen-CutieCon.loadAuthInfo(Session.deCrypt(config.SESSION)); 
    } else {
        Queen-CutieCon.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
    }

if (config.LANG == 'EN' || config.LANG == 'ES') {
    Queen-CutieCon.on ('credentials-updated', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login details updated!')
        );

        const authInfo = Queen-CutieCon.base64EncodedAuthInfo();
        if (StrSes_Db.length < 1) {
            await Queen-CutieDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
if (config.LANG == 'EN' || config.LANG == 'ES') {
    Queen-CutieCon.on('connecting', async () => {
...
if (config.WORKTYPE == 'public') {
    if (Queen-CutieCon.user.jid === '@s.whatsapp.net') {

        await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '```🛡️ Blacklist Detected!``` \n```User:```  \n```Reason:``` ', MessageType.text)
    ...
    if (Queen-CutieCon.user.jid === '@s.whatsapp.net') {
        ...
        await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*Queen Amdi started in Private Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Private Mode. To change, use_ ```.setvar WORK_TYPE=public``` _command._\n\n*You can check our website for new features : https://www.amdaniwasa.com/*\n\n*Comment here to request new features:*\nhttps://amdaniwasa.com/index-en.html#comments\n\n*Updates:* https://gist.github.com/BlackAmda/890b6b31fcb8d376d6a68afcb7359324\n\n *Thank you for using Queen Amdi 💌*', MessageType.text);
    }
}
    else {
    ...
    Queen-CutieCon.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
                ...
                if (Queen-CutieCon.user.jid === '@s.whatsapp.net') {
                    ...
                    await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '```🛡️ Blacklist අනාවරණය විය!``` \n```පරිශීලක:``` \n```හේතුව:``` ', MessageType.text)
                ...
                await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*Queen ඇම්ඩි private ආකාරයට ක්‍රියාකිරිම ආරම්භ විය.👸*\n\n_කරුණාකර මෙහි command උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n_ඔබේ command list එක ලබාගැනීමට_ *.panel* _command එක භාවිතා කරන්න._\n\n_ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට_ ```.setvar WORK_TYPE=public``` _විධානය භාවිතා කරන්න._\n\n*නව විශේෂාංග සඳහා ඔබට අපගේ වෙබ් අඩවිය පරීක්‍ෂා කළ හැකිය : https://www.amdaniwasa.com/*\n\n*Queen Amdi සඳහා එකතු කළ හැකි නව අදහස් මෙහි comment කරන්න:*\nhttps://amdaniwasa.com/index-si.html#comments\n\n*Updates:* https://gist.github.com/BlackAmda/890b6b31fcb8d376d6a68afcb7359324\n\n *Queen ඇම්ඩි භාවිතා කිරීම ගැන ස්තූතියි 💌*', MessageType.text);
                }
            }
...
// ==============================================================
    ...
    Queen-CutieCon.on('message-new', async msg => {
        if (msg.key && msg.key.remoteJid == 'status@broadcast') return;

        if (config.NO_ONLINE) {
            await Queen-CutieCon.updatePresence(msg.key.remoteJid, Presence.unavailable);
        }
        ...
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                await Queen-CutieCon.sendMessage(msg.key.remoteJid, Buffer.from (blogo.data), MessageType.image, {mimetype: Mimetype.png, caption: gb.message});
            }
            return;
            ...
            if (gb !== false) {
                await Queen-CutieCon.sendMessage(msg.key.remoteJid, Buffer.from (wlogo.data), MessageType.image, {mimetype: Mimetype.png, caption: gb.message});
            }
            return;
        }
        // ====================================================================
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    let sendMsg = false;
                    var chat = Queen-CutieCon.chats.get(msg.key.remoteJid)
                    ...
                    if (sendMsg) {
                        if (config.SEND_READ && command.on === undefined) {
                            await Queen-CutieCon.chatRead(msg.key.remoteJid);
                        }
                        ...
                        && msg.message.imageMessage !== null) {
                            whats = new Image(Queen-CutieCon, msg);
                        } else if (command.on !== undefined && (command.on === 'video' )
                        && msg.message.videoMessage !== null) {
                            whats = new Video(Queen-CutieCon, msg);
                        } else {
                            whats = new Message(Queen-CutieCon, msg);
                        }
                        if (msg.key.fromMe) {
                            var vers = Queen-CutieCon.user.phone.wa_version.split('.')[2]
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*-- දෝෂ වාර්තාව [QUEEN AMDI] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [QUEEN AMDI] ⚜️*' + 
                                        '\n==== ```දෝෂ නිරාකරණය කර ඇත!``` ====' +
                                    ...
                                    return await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [QUEEN AMDI] ⚜️*' + 
                                        '\n==== ```දෝෂ නිරාකරණය කර ඇත!``` ====' +
                                        '\n\n*ප්‍රධාන දෝෂය:* _Ookla Server Queen-CutieConection_' +
                                    ...
                                    return await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*🙇🏻 කණගාටුයි, මට මෙම දෝෂය කියවිය නොහැක! 🙇🏻*' +
                                        '\n_වැඩිදුර උදව් සඳහා ඔබට අපගේ support කණ්ඩායම් වෙත ලිවිය හැකිය._'
                                        , MessageType.text
                                    );
                                }
                            } else {
                                await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*-- ERROR REPORT [QUEEN AMDI] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*⚜️ ERROR ANALYSIS [QUEEN AMDI] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*⚜️ ERROR ANALYSIS [QUEEN AMDI] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                        '\n\n*Main Error:* _Ookla Server Queen-CutieConection_' +
                                    ...
                                    return await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*⚜️ ERROR ANALYSIS [QUEEN AMDI] ⚜️*' + 
                                        ...
                                        '\n\n*Reason:* _Queen-CutieConection between site disQueen-CutieConected._' +
                                    ...
                                    return await Queen-CutieCon.sendMessage(Queen-CutieCon.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
