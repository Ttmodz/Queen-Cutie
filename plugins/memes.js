...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'meme ?(.*)', fromMe: true, desc: Lang.MEMES_DESC}, (async (message, match) => {   
	    ...
	    memeMaker({
            image: location,         
            outfile: 'Queen-Cutie-meme.png',
            topText: topText,
            bottomText: bottomText,
        }, async function(err) {
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('Queen-Cutie-meme.png'), MessageType.image, {filename: 'Queen-Cutie-meme.png', mimetype: Mimetype.png, caption: Config.AFN});
            await info.delete();    
        });
    }));
}
