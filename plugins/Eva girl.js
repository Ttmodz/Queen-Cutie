...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('QueenCutie') && conf.TALKING_PINKY !== 'true') {        
        ...
        var finm = message.message.replace('QueenCutie', '').replace(' ', '')   
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel QueenCutie özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'QueenCutie yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'QueenCutie yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'QueenCutie, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'QueenCutie, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional QueenCutie features. Turn your account into a ai chatbot!'
...
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ QueenCutie സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'QueenCutie കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'QueenCutie AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
...
Asena.addCommand({ pattern: 'QueenCutie ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.QueenCutie on / off' }, (async (message, match) => {
