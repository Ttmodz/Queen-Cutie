...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to Cutie voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Cutie') && conf.TALKING_CUTIE !== 'true') {        
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_CUTIE == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Cutie özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Cutie yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Cutie yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Cutie, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Cutie, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Cutie features. Turn your account into a ai chatbot!'
    already_on = 'Cutie artificial intelligence is already fully functional.'
    already_off = 'Cutie artificial intelligence is currently running semi-functional.'
    succ_on = 'Cutie Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Cutie Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Cutie സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Cutie കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Cutie AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Cutie പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Cutie സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'Cutie ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.Cutie on / off' }, (async (message, match) => {
    var Cutie_status = `${conf.TALKING_CUTIE}`
    if (match[1] == 'on') {
        if (Cutie_status == 'true') {
