...
if (Config.WORKTYPE == 'private') {
    ...
    MyPnky.addCommand({pattern: 'meme ?(.*)', fromMe: true, desc: Lang.MEMES_DESC}, (async (message, match) => {   
	    ...
	    memeMaker({
            image: location,         
            outfile: 'QueenCutie-meme.png',
            topText: topText,
            bottomText: bottomText,
        }, async function(err) {
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('QueenCutie-meme.png'), MessageType.image, {filename: 'QueenCutie-meme.png', mimetype: Mimetype.png, caption: Config.AFN});
            await info.delete();    
        });
    }));
}
    ...
    MyPnky.addCommand({pattern: 'meme ?(.*)', fromMe: false, desc: Lang.MEMES_DESC}, (async (message, match) => {    
	 ...
	 if (match[1].includes('{1}')) {    
	    ...
	    fontSize: 150, 	
            ...
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('plk-QueenCutie.png'), MessageType.image, {filename: 'QueenCutie_meme.png', mimetype: Mimetype.png, caption: Config.AFN});
            await info.delete();    
        });
       }
